...
DO $$ 
BEGIN
  -- Only drop and recreate role column if it exists and is not the correct type
  IF EXISTS (
    ...
    ALTER TABLE admin_users ADD COLUMN role admin_role NOT NULL DEFAULT 'shop_admin';
  END IF;

  -- Only add shop_id if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'admin_users' 
    AND column_name = 'shop_id'
  ) THEN
    ALTER TABLE admin_users ADD COLUMN shop_id uuid REFERENCES shops(id);
  END IF;
END $$;
